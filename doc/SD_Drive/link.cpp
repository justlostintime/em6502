...
// STROBE - From the master.  Indicates either data is available if host is
//          sending, or ACK if we're sending.
...
void Link::stateMachine(word token)
{
        ...
        switch (state)
        {
                ...
                case STATE_GET_ONE:
                        ...
                        if (event->getType() == EVT_WRITE_SECTOR)
                        {
...
void Link::sendEvent(Event *eptr)
{
        ...
        switch (eptr->getType())
        {
                ...
                case EVT_READ_SECTOR:
                {
                        ...
                        //Serial.println("sending data over port: ");
                ...
                case EVT_CLOCK_DATA:
                Serial.print("sending clock data ");
                        writeByte(PROTO_CLOCK_DATA);
                        byte *dptr = eptr->getData();
                        for (int i = 0; i < 8; i++)
                        {
                          Serial.print(*dptr, DEC);
                          Serial.print(" ");
                                writeByte(*dptr++);
                        }
                        Serial.println("");
