...
code_segment equ $400  

;self modifying code may be disabled to allow running in ROM
;0=part of the code is self modifying and must reside in RAM
;1=tests disabled: branch range
disable_selfmod equ 0
...
;masking of always on bits after PHP or BRK (unused & break) on compare
    if disable_decimal < 2
        if I_flag = 0
load_flag   macro
            lda #{0}&m8i         ;force enable interrupts (mask I)
            endm
cmp_flag    macro
            cmp #({0}|fao)&m8i   ;I_flag is always enabled + always on bits
            endm
eor_flag    macro
            eor #({0}&m8i|fao)   ;mask I, invert expected flags + always on bits
            endm
        endif
        if I_flag = 1
load_flag   macro
            lda #{0}|intdis      ;force disable interrupts
            endm
cmp_flag    macro
            cmp #({0}|fai)&m8    ;I_flag is always disabled + always on bits
            endm
eor_flag    macro
            eor #({0}|fai)       ;invert expected flags + always on bits + I
            endm
        endif
        if I_flag = 2
load_flag   macro
...
cmp_flag    macro
            ...
            cmp #({0}|fao)&m8i   ;expected flags + always on bits, mask I
            endm
eor_flag    macro
            eor flag_I_on       ;I_flag is never changed
            eor #({0}&m8i|fao)   ;mask I, invert expected flags + always on bits
...
cmp_flag    macro
            ...
            cmp #({0}|faod)&m8i  ;I_flag is always enabled + always on bits
            endm
eor_flag    macro
            ora #decmode        ;ignore decimal mode bit
            eor #({0}&m8i|faod)  ;mask I, invert expected flags + always on bits
            endm
        endif
        if I_flag = 1
load_flag   macro
...
cmp_flag    macro
            ora #decmode        ;ignore decimal mode bit
            cmp #({0}|faid)&m8   ;I_flag is always disabled + always on bits
            endm
eor_flag    macro
            ora #decmode        ;ignore decimal mode bit
            eor #({0}|faid)      ;invert expected flags + always on bits + I
            endm
        endif
        if I_flag = 2
load_flag   macro
...
cmp_flag    macro
            eor flag_I_on       ;I_flag is never changed
            ora #decmode        ;ignore decimal mode bit
            cmp #({0}|faod)&m8i  ;expected flags + always on bits, mask I
            endm
eor_flag    macro
            eor flag_I_on       ;I_flag is never changed
            ora #decmode        ;ignore decimal mode bit
...
cmp_flag    macro
            ora #decmode        ;ignore decimal mode bit
            cmp #({0}|faod)&m8   ;expected flags + always on bits
            endm
eor_flag    macro
            ora #decmode        ;ignore decimal mode bit
            eor #{0}|faod        ;invert expected flags + always on bits
            endm
        endif
    endif
...
set_zx      macro       ;precharging zp,x & immediate status
            load_flag {1}
            pha         ;use stack to load status
            lda {0},x    ;load to indexed zeropage
            sta zpt,x
            plp
            endm
...
set_absx    macro       ;precharging abs,x & immediate status
            load_flag {1}
            pha         ;use stack to load status
            lda {0},x    ;load to indexed memory
            sta abst,x
            plp
            endm
